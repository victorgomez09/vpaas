...
export class ManagerService {
  ...
  initialize() {
    ...
    this.refreshForceSslState()
      ...
      .then(function () {
        Logger.debug('Emptying generated and temp folders.');
      ...
      .then(function () {
        ...
        EventLoggerFactory.get(
          ...
          .getLogger()
          .trackEvent(
            CapRoverEventFactory.create(CapRoverEventType.InstanceStarted, {}),
          );
  ...
  isInitialized() {
    return (
      this.inited && !this.waitUntilRestarted && !this.backupManager.isRunning()
    );
  }
    ...
    return Promise.resolve() //
      ...
      .then(function (apps) {
        ...
        const serviceManager = ServiceManager.get(
          ...
          Authenticator.getAuthenticator(self.dataStore.getNameSpace()),
          self.dataStore,
          self.dockerApi,
      ...
      .then(function () {
        if (netDataInfo.isEnabled) {
          ...
          if (netDataInfo.data.smtp) {
            ...
            envVars.push({
              key: 'SMTP_STARTTLS',
              value: netDataInfo.data.smtp.allowNonTls ? '' : 'on',
            });
          }
  ...
  changeCaptainRootDomain(requestedCustomDomain: string, force: boolean) {
    ...
    return self.domainResolveChecker
      ...
      .then(function () {
        return self.dataStore.getRegistriesDataStore().getAllRegistries();
      })
      .then(function (registries) {
        let localRegistry: IRegistryInfo | undefined = undefined;
