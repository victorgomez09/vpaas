	...
	{#if id !== 'new'}
		<nav class="flex flex-1 items-center justify-between">
			...
			<div class="flex flex-row flex-wrap space-x-3 justify-center lg:justify-start lg:py-0">
				{#if database.type && database.destinationDockerId && database.version}
					{#if $status.database.isExited}
						<a
							...
							class="icons bg-transparent text-error tooltip-error"
						>
							<svg
					...
					{/if}
				{/if}
				<div class="border bg-base-300 h-8" />
				<a
					id="configuration"
					href="/databases/{id}"
					class="hover:text-warning rounded"
					class:text-warning={$page.url.pathname === `/databases/${id}`}
					class:bg-base-300={$page.url.pathname === `/databases/${id}`}
				>
					<button class="icons bg-transparent m text-sm disabled:text-error">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-6 w-6"
							viewBox="0 0 24 24"
				...
				<div class="border border-base-300 h-8" />
				<a
					id="databaselogs"
					href={$status.database.isRunning ? `/databases/${id}/logs` : null}
					class="hover:text-warning rounded"
					class:text-warning={$page.url.pathname === `/databases/${id}/logs`}
					class:bg-base-300={$page.url.pathname === `/databases/${id}/logs`}
				>
					<button disabled={!$status.database.isRunning} class="icons text-sm cursor-pointer">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-6 w-6"
							viewBox="0 0 24 24"
