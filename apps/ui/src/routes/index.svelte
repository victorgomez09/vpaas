		...
		{#if applications.length !== 0 || destinations.length !== 0 || databases.length !== 0 || services.length !== 0 || gitSources.length !== 0 || destinations.length !== 0}
			<div class="space-x-2 lg:flex lg:justify-center text-center mb-4">
				<button
					...
					class:text-secondary={$search === '!app'}
					class:bg-base-300={$search === '!app'}
					class:hover:bg-base-300={$search !== '!app'}
					class:hover:text-secondary={$search !== '!app'}
				...
				<button
					class="btn btn-sm btn-ghost"
					class:text-secondary={$search === '!service'}
					class:bg-base-300={$search === '!service'}
					class:hover:bg-base-300={$search !== '!service'}
					class:hover:text-secondary={$search !== '!service'}
				...
				<button
					class="btn btn-sm btn-ghost"
					class:text-secondary={$search === '!db'}
					class:bg-base-300={$search === '!db'}
					class:hover:bg-base-300={$search !== '!db'}
					class:hover:text-secondary={$search !== '!db'}
				...
				<button
					class="btn btn-sm btn-ghost"
					class:text-secondary={$search === '!git'}
					class:bg-base-300={$search === '!git'}
					class:hover:text-secondary={$search !== '!git'}
					class:hover:bg-base-300={$search !== '!git'}
				...
				<button
					class="btn btn-sm btn-ghost"
					class:text-secondary={$search === '!destination'}
					class:bg-base-300={$search === '!destination'}
					class:hover:text-secondary={$search !== '!destination'}
					class:hover:bg-base-300={$search !== '!destination'}
			...
			<div class="form-control">
				<div class="input-group flex w-full gap-2">
					...
					<button
						class="btn no-animation hover:bg-error"
						on:click={() => doSearch('')}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
				...
				{#if filtered.applications.length > 0}
					{#each filtered.applications as application}
						<a class="no-underline mb-5" href={`/applications/${application.id}`}>
							<div
								class="w-full rounded p-5 bg-base-200 hover:bg-base-300 hover:scale-105 indicator duration-150"
							>
									...
									<div class="w-full flex flex-col">
										<h1 class="font-bold text-base truncate">
											...
											{#if application.settings?.isBot}
												<span class="text-xs badge border-none text-success"
													>BOT</span
												>
											{/if}
										</h1>
										<div class="h-10 text-xs mt-2 italic space-y-1">
											{#if application?.fqdn}
												<div class="truncate">
											...
											{:else if !application.settings?.isBot && !application?.fqdn && application.buildPack !== 'compose'}
												<h2 class="text-error">Not configured</h2>
											{/if}
					...
					{#each filtered.services as service}
						{#key service.id}
							<a class="no-underline mb-5" href={`/services/${service.id}`}>
								<div
									class="w-full rounded p-5 shadow bg-base-200 hover:bg-base-300 hover:scale-105 indicator duration-150"
								>
									...
									<div class="w-full flex flex-row">
										...
										<div class="w-full flex flex-col">
											...
											<div class="h-10 text-xs font-extralight space-y-1 mt-2 italic">
												{#if service?.fqdn}
													<h2>
														<span class="font-semibold">Url: </span>
												...
												{:else}
													<h2 class="text-error">URL not configured</h2>
												{/if}
											...
											<div class="flex justify-end items-end space-x-2 h-10">
												...
												{#if $appSession.isAdmin}
													<button
														class="icons hover:text-error"
														on:click|stopPropagation|preventDefault={() =>
															deleteService(service.id)}><DeleteIcon /></button
													>
												{/if}
											</div>
										</div>
									...
									<div class="w-full flex flex-row">
										...
										<div class="w-full flex flex-col">
											<div class="h-10">
												...
												<div class="h-10 text-xs mt-2 space-y-1 italic">
													{#if database?.version}
														<div class="truncate">
													...
													{:else}
														<h2 class="text-error">Not version not configured</h2>
													{/if}
					...
					{#each filtered.destinations as destination}
						{#key destination.id}
							<a class="no-underline mb-5" href={`/destinations/${destination.id}`}>
								<div
									class="w-full rounded p-5 bg-base-200 shadow hover:bg-base-300 hover:scale-105 indicator duration-150"
								>
									<div class="w-full flex flex-row">
										<div class="absolute top-0 left-0 -m-5 h-10 w-10">
											...
											{#if destination.remoteEngine}
												<svg
													xmlns="http://www.w3.org/2000/svg"
													class="absolute top-0 left-9 -m-2 h-6 w-6 text-info rotate-45"
													viewBox="0 0 24 24"
													stroke-width="3"
													stroke="currentColor"
													fill="none"
										...
										<div class="w-full flex flex-col">
											...
											<div class="h-10 text-xs font-extralight italic mt-2">
												{#if $appSession.teamId === '0' && destination.remoteVerified === false && destination.remoteEngine}
													<h2 class="text-error">Not verified yet</h2>
												{/if}
												{#if destination.remoteEngine && !destination.sshKeyId}
													<h2 class="text-error">SSH key missing</h2>
												{/if}
												{#if destination.teams.length > 0 && destination.teams[0]?.name}
													<div class="truncate">
														<span class="font-semibold">Team:</span> {destination.teams[0]?.name}
													</div>
												{/if}
											</div>
										</div>
