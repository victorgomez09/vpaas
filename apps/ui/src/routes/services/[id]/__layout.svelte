...
<div class="flex flex-col flex-1 mt-4">
	<div class="px-6 flex flex-1 items-center justify-between">
		...
		<div
			class="flex flex-row items-start justify-center lg:justify-end space-x-2 order-1 lg:order-2"
		>
	...
	<div
		class="mt-10 mx-auto max-w-screen-2xl px-0 lg:px-10 grid grid-cols-1"
		class:lg:grid-cols-4={!$page.url.pathname.startsWith(`/services/${id}/configuration/`)}
	>
		{#if !$page.url.pathname.startsWith(`/services/${id}/configuration/`)}
			<nav class="flex flex-col lg:pt-0">
				<Menu {service} {template} />
			</nav>
		{/if}
		<div class="pt-0 ml-4 col-span-0 lg:col-span-3 pb-24 px-4 lg:px-0">
			<slot />
		</div>
	</div>
</div>
