	...
	if (settings) {
		...
		try {
			if (!applicationStoragePathMigrationFinished) {
				...
				for (const application of applications) {
					if (
						...
						application.persistentStorage.length > 0 &&
						application?.buildPack !== 'docker'
					) {
			...
			await prisma.setting.update({
				where: { id: settingsId },
				data: { applicationStoragePathMigrationFinished: true }
			});
		}
	}
}
		...
		for (const service of services) {
			try {
				...
				if (template) {
					...
					if (service.type === 'plausibleanalytics' && service.plausibleAnalytics)
						await plausibleAnalytics(service, template);
					...
					if (service.type === 'vscodeserver' && service.vscodeserver)
						await vscodeserver(service, template);
					...
					if (service.type === 'meilisearch' && service.meiliSearch)
						await meilisearch(service, template);
...
async function searxng(service: any, template: any) {
	...
	const secrets = [`SECRET_KEY@@@${secretKey}`, `REDIS_PASSWORD@@@${redisPassword}`];

	const settings = [`SEARXNG_BASE_URL@@@$$generate_fqdn`];
	await migrateSettings(settings, service, template);
	await migrateSecrets(secrets, service);
...
async function glitchtip(service: any, template: any) {
	...
	const secrets = [
		...
		emailSmtpUser &&
			...
			emailSmtpPort &&
			`EMAIL_URL@@@${encrypt(
				`smtp://${emailSmtpUser}:${decrypt(emailSmtpPassword)}@${emailSmtpHost}:${emailSmtpPort}`
			)} || ''`,
		`DATABASE_URL@@@${encrypt(
			`postgres://${postgresqlUser}:${decrypt(
				postgresqlPassword
			)}@${id}-postgresql:5432/${postgresqlDatabase}`
		)}`,
...
async function hasura(service: any, template: any) {
	const { postgresqlUser, postgresqlPassword, postgresqlDatabase, graphQLAdminPassword } =
		service.hasura;
	const { id } = service;

	const secrets = [
		`HASURA_GRAPHQL_ADMIN_SECRET@@@${graphQLAdminPassword}`,
		`HASURA_GRAPHQL_METADATA_DATABASE_URL@@@${encrypt(
			`postgres://${postgresqlUser}:${decrypt(
				postgresqlPassword
			)}@${id}-postgresql:5432/${postgresqlDatabase}`
		)}`,
	...
	const settings = [`POSTGRES_USER@@@${postgresqlUser}`, `POSTGRES_DB@@@${postgresqlDatabase}`];
	await migrateSettings(settings, service, template);
	await migrateSecrets(secrets, service);
...
async function umami(service: any, template: any) {
	const { postgresqlUser, postgresqlPassword, postgresqlDatabase, umamiAdminPassword, hashSalt } =
		service.umami;
	const { id } = service;
	const secrets = [
		`HASH_SALT@@@${hashSalt}`,
...
async function meilisearch(service: any, template: any) {
	...
	const secrets = [`MEILI_MASTER_KEY@@@${masterKey}`];
...
async function wordpress(service: any, template: any) {
	...
	if (ownMysql) {
		secrets = [
			...
			ftpPassword && `VPAAS_FTP_PASSWORD@@@${ftpPassword}`,
			ftpHostKeyPrivate && `VPAAS_FTP_HOST_KEY_PRIVATE@@@${ftpHostKeyPrivate}`,
			ftpHostKey && `VPAAS_FTP_HOST_KEY@@@${ftpHostKey}`
		];
		settings = [
		...
		settings = [
			...
			`VPAAS_OWN_DB@@@${ownMysql}`,
			`VPAAS_FTP_ENABLED@@@${ftpEnabled}`,
			`VPAAS_FTP_USER@@@${ftpUser}`,
			`VPAAS_FTP_PUBLIC_PORT@@@${ftpPublicPort}`
		];
	}
...
async function minio(service: any, template: any) {
	...
	const secrets = [`MINIO_ROOT_PASSWORD@@@${rootUserPassword}`];
	const settings = [
...
async function fider(service: any, template: any) {
	...
	const secrets = [
		...
		`POSTGRES_PASSWORD@@@${postgresqlPassword}`,
		`DATABASE_URL@@@${encrypt(
			`postgresql://${postgresqlUser}:${decrypt(
				postgresqlPassword
			)}@${id}-postgresql:5432/${postgresqlDatabase}?sslmode=disable`
		)}`
	];
	const settings = [
...
async function migrateSettings(settings: any[], service: any, template: any) {
	for (const setting of settings) {
		try {
			...
			(await prisma.serviceSetting.findFirst({ where: { name: minio, serviceId: service.id } })) ||
				(await prisma.serviceSetting.create({
					data: { name: minio, value, variableName, service: { connect: { id: service.id } } }
				}));
		} catch (error) {
			console.log(error);
		}
...
async function migrateSecrets(secrets: any[], service: any) {
	for (const secret of secrets) {
		try {
			...
			(await prisma.serviceSecret.findFirst({ where: { name, serviceId: service.id } })) ||
				(await prisma.serviceSecret.create({
					data: { name, value, service: { connect: { id: service.id } } }
				}));
		} catch (error) {
			console.log(error);
		}
...
async function createVolumes(service: any, template: any) {
	...
	for (const volume of volumes) {
		...
		(await prisma.servicePersistentStorage.findFirst({
			where: { volumeName, serviceId: service.id }
		})) ||
			(await prisma.servicePersistentStorage.create({
				data: {
					volumeName,
