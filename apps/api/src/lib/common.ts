...
export async function isDomainConfigured({
	...
	const vpaasFqdn = await prisma.setting.findFirst({
		where: {
			OR: [
				{ fqdn: { endsWith: `//${nakedDomain}` } },
				{ fqdn: { endsWith: `//www.${nakedDomain}` } }
	...
	return !!(foundApp || foundService || vpaasFqdn);
}
...
export async function createRemoteEngineConfiguration(id: string) {
	...
	config.append({
		...
		ControlMaster: 'auto',
		ControlPath: `${homedir}/.ssh/vpaas-${remoteIpAddress}-%r@%h:%p`,
		ControlPersist: '10m'
	});
...
export async function startTraefikProxy(id: string): Promise<void> {
	...
	if (!found) {
		const { stdout: vpaasNetwork } = await executeCommand({
			dockerId: id,
			command: `docker network ls --filter 'name=vpaas-infra' --no-trunc --format "{{json .}}"`
		});
...
export async function configureNetworkTraefikProxy(destination: any): Promise<void> {
	...
	const { stdout: networks } = await executeCommand({
		dockerId: id,
		command: `docker ps -a --filter name=vpaas-proxy --format '{{json .Networks}}'`
	});
	const configuredNetworks = networks.replace(/"/g, '').replace('\n', '').split(',');
	if (!configuredNetworks.includes(destination.network)) {
		await executeCommand({
			dockerId: destination.id,
			command: `docker network connect ${destination.network} vpaas-proxy`
		});
	}
}
...
export async function stopTraefikProxy(
	...
	const { found } = await checkContainer({ dockerId: id, container: 'vpaas-proxy' });
	await prisma.destinationDocker.update({
		where: { id },
	...
	try {
		if (found) {
			await executeCommand({
				dockerId: id,
				command: `docker stop -t 0 vpaas-proxy && docker rm vpaas-proxy`,
				shell: true
			});
		}
	} catch (error) {
...
export async function makeLabelForStandaloneDatabase({ id, image, volume }) {
	...
	return [
		...
		`vpaas.name=${database.name}`,
		`vpaas.configuration=${base64Encode(
			JSON.stringify({
				version,
				image,
				volume,
		...
		const interval = setInterval(async () => {
			try {
				...
				const { stdout: buildContainers } = await executeCommand({
					dockerId,
					command: `docker container ls --filter "label=vpaas.buildId=${buildId}" --format '{{json .}}'`
				});
				if (buildContainers) {
