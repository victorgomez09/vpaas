	...
	try {
		...
		if (database) {
			...
			std = await executeCommand({
				dockerId: database.destinationDockerId,
				command: `docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v vpaas-local-backup:/app/backups -e CONTAINERS_TO_BACKUP="${backupData}" coollabsio/backup`
			});
		}
		if (std.stdout) {
			return std.stdout;
...
export async function restartVpaas(request: FastifyRequest<any>) {
	try {
		...
		if (teamId === '0') {
			if (!isDev) {
				await executeCommand({ command: `docker restart vpaas` });
				return {};
			} else {
				return {};
			}
					...
					data: {
						...
						teams: {
							create: {
								id: uid,
								name: uniqueName(),
								destinationDocker: { connect: { network: 'vpaas' } }
							}
						},
