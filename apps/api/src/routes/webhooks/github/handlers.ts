...
import {
	createdBranchDatabase,
	getApplicationFromDBWebhook,
	removeBranchDatabase
} from '../../api/v1/applications/handlers';

import type { FastifyReply, FastifyRequest } from 'fastify';
import type { GitHubEvents, InstallGithub } from './types';

export async function installGithub(
	request: FastifyRequest<InstallGithub>,
	reply: FastifyReply
): Promise<any> {
	try {
...
export async function gitHubEvents(request: FastifyRequest<GitHubEvents>): Promise<any> {
	try {
		...
		if (!projectId || !branch) {
			throw {
				status: 500,
				message: 'Cannot parse projectId or branch from the webhook?!',
				type: 'webhook'
			};
		}
					...
					if (application.settings.previews) {
						...
						if (
							...
							if (pullmergeRequestId) {
								const foundPreviewApplications = await prisma.previewApplication.findMany({
								...
								if (foundPreviewApplications.length > 0) {
									...
									const previewApplication = await prisma.previewApplication.create({
										data: {
											pullmergeRequestId,
											sourceBranch,
											customDomain: `${protocol}${pullmergeRequestId}${
												settings.previewSeparator
